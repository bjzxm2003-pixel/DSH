...
- **选题防重复（三层）**：①搜索词按日期轮换，不同天搜不同组合；②选题时把近 25 条已写标题喂给 LLM，要求避开同题和"换皮"选题（要求出 5 条、查重后取 3 条）；③程序级相似度兜底——新选题与全部历史标题比对，完全相同 / 4 字以上片段互含 / bigram 相似度>0.4 / 共享 4 字以上实义片段（如专名「永乐大典」）均判重复剔除，成稿前再查重一次。
